...
###################
#the first situation: -1 inside: have dimension no select feature%%%%%%%select the important features
#the second one: one feature is used twice%%%%%%%%%%%%%%%%%%%plus/minus the interval
#the third one: all dimensions select features%%%%%%%%%%%%remove one feature

################the key idea is randomly select one dimension without feature is selected, then add a new feature
...
#     # predomi = [tt[-1] for tt in space]###the set of the fetaure that is the most important one in each group
#     for i in range(len(new)):
#         mic_solution = all_mic[i]
#         x = find_selected_index(new[i],space,all_pre_intervals)
#         basic_sf = fit_train1(new[i], x_train, y, space, all_pre_intervals)
...
#         # print(index_all_minus1)
#         # print(unique_x)
#         # exit()
#         if len(index_all_minus1) == 0:###all dimensions use to select feaures,
#             print('very special case',x)
#             index_temp = []
#             for in1 in unique_x:
...
#                 temp = 0
#                 for id2 in index2:
#                     random_dimension = index_temp[id2][0]
...
#                         position = np.argwhere(all_features == remaining_features[add_one])
#                         position = position[0][0]
#                         # print(used_index, add_one,position )
#                         ref_di = all_pre_intervals[random_dimension]
#                         nei3[random_dimension] = random.uniform((position + 1) * (
#                                 ref_di[1] - ref_di[0]), (position + 2) * (
#                                                                         ref_di[1] - ref_di[0]))
#                 if temp > 0:
...
#             #       pop_new[i] = nei2
#             #       pop_fit[i] = nb2_sf
#             ##############################################################the above version didn't use removing operator
#         else:######have dimension whose feature haven't selected
#             if len(unique_x) == len(x_useful): ##that means no feature is used twice
#                 mic_solution = all_mic[i]
#                 random_dimension = random.sample(index_all_minus1, 1)  ###the selected dimension
#                 random_dimension = random_dimension[0]
#                 nei3,used_index = neighhood_minus_1(new[i],mic_solution,x,space,all_pre_intervals,mic_value,index_all_minus1,random_dimension)
#                 if len(used_index) > 0:####that means new[i] changes
#                     nb3_sf = fit_train1(nei3, x_train, y, space, all_pre_intervals)
#                     fit_count = fit_count + 1
#                     if nb3_sf < basic_sf:
#                             pop_new[i] = nei3
#                             pop_fit[i] = nb3_sf
#             else:#####have features are use twice
#                 index_temp = []
#                 for in1 in unique_x:
#                     id1 = [j1 for j1, j2 in enumerate(x) if j2== in1]
#                     index_temp.append(id1)
#                 index2 = [j1 for j1, j2 in enumerate(index_temp) if len(j2)>1]
#                 temp = 0
#                 for id2 in index2:
#                     random_dimension = index_temp[id2][0]
